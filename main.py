...
app.mount("/static", StaticFiles(directory="siplygo_app/static"), name="static")

# Jinja2 environment for rendering HTML templates
templates_env = Environment(
    loader=FileSystemLoader("siplygo_app/templates"),
...
# A single in-memory cart for demonstration.  In a real application, carts would
# be per-user and persisted in a database or session.
demo_cart = Cart()
...
# We deliberately avoid using Starlette's built-in sessions because installing
# the required dependencies isn't possible in this environment.  The demo uses a
# single global cart instead.
