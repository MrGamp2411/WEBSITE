    ...
    try:
        ...
        for b in db.query(BarModel).all():
            ...
            bar = Bar(
                ...
                photos=json.loads(b.photos) if b.photos else [],
...
async def create_bar_post(request: Request, db: Session = Depends(get_db)):
    ...
    rating_count = form.get("rating_count")
    ...
    photos_json = (
        json.dumps([p.strip() for p in photos.split(",") if p.strip()]) if photos else None
    )
    hours = {}
    for i in range(7):
...
async def edit_bar_form(request: Request, bar_id: int, db: Session = Depends(get_db)):
    ...
    photos_csv = ", ".join(json.loads(bar.photos)) if bar.photos else ""
...
async def edit_bar_post(request: Request, bar_id: int, db: Session = Depends(get_db)):
    ...
    if all([name, address, city, state, latitude, longitude, description]):
        ...
        bar.rating_count = int(rating_count) if rating_count else 0
        ...
        if mem_bar:
            ...
            mem_bar.rating_count = int(rating_count) if rating_count else 0
            ...
            mem_bar.photos = (
                [p.strip() for p in photos.split(",") if p.strip()] if photos else []
            )
