...
- Admin Audit Logs page uses `templates/admin_audit_logs.html` with `.users-page` layout and filters for username, bar name, and action. Action filter is a dropdown and filters apply automatically on change. The table shows usernames and bar names instead of numeric IDs.
