...
- Admin Manage Payments page uses `templates/admin_payments.html` with `.payments-page` styles in `static/css/components.css`, a client-side bar search via `#paymentsSearch`, and grouped action pills (`.btn-outline` for View/Add Test Closing, `.btn-danger-soft` for Delete Test Closing). The page header stacks the title above the search controls
